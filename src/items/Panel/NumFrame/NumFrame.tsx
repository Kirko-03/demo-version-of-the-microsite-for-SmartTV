...
export const NumFrame = (props:NumFrame) =>{
    ...
    return(
        <div className={nf.numFrame}>
            {array.map(el=>el.length>1?<NumberButton width={'182px'} onClick={()=>props.setArrayNumber([])} text={el}/>:<NumberButton onClick={()=>props.setArrayNumber([...props.arrayNumber,el].filter(el=>el!=='-'))} width={'88px'}  text={el}/>)}
        </div>)
}